...
class LogService:
    ...
    def parse_log(self, w: dict, tail: int = 500) -> list[dict]:
        ...
        for line in lines:
            ...
            if "append_task" in line:
                for k, v in task_map.items():
                    if k in line:
                        cur_task = {"name": v, "start": ts, "status": "运行中", "drops": "", "error": ""}
                        tasks.append(cur_task)
                        break
            elif "[ERR]" in line and cur_task:
                cur_task["status"] = "失败"
                cur_task["error"] = line.split("[ERR]")[-1].strip()[:100]
            elif "TaskSwitched" in line and cur_task:
                cur_task["status"] = "完成"
            elif "StageDrops" in line and cur_task:
                drops = re.findall(r"\b(\S+?)\s*[xX×]\s*(\d+)", line)
                if drops:
                    cur_task["drops"] = ",".join(f"{d[0]}x{d[1]}" for d in drops[-5:])
        return tasks

    def show_stats(self, w: dict) -> None:
        tasks = self.parse_log(w)
        if not tasks:
            QMessageBox.information(self.ctx._mw, "统计", "暂无运行数据\n等待 MAA 执行任务后自动生成")
            return
        d = QDialog(self.ctx._mw)
        d.setWindowTitle("MAA 运行统计")
        d.setMinimumSize(400, 300)
        l = QVBoxLayout(d)
        l.addWidget(QLabel(f"📊 MAA 运行统计 ({len(tasks)} 个任务)", font=QFont("Microsoft YaHei UI", 13, QFont.Bold)))
        tw = QTableWidget()
        tw.setColumnCount(3)
