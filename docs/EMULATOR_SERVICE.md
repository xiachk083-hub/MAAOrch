...
## 9.4 第四轮评审补充（2026-08-12）— 未发生但必然发生的场景

### 9.4.1 预热取消的壳风险

- PREWARMING 中途取消（队列清空/账号移除/并行上限下调）→ **不中途杀**（半启动 VMM 安卓未起 → 关不掉 → 变壳）
- 规则：取消 = 标记 `cancel_pending` → **等启动自然完成**（PREWARMING → READY）→ **立即回收**（CLOSING）——全程不产生壳
- 预热启动完成前禁止任何 taskkill（除非超过启动超时 120s——按壳处理）

### 9.4.2 并发启动节流

- 同时 PREWARMING 上限：`prewarm_concurrency`（默认 2——吸收 2026-08-11 "10 台同时失联误报"教训）
- 超限的预热请求排队（事件循环内等待，不并发）
- 冷启动（acquire 时 OFF）同样受此节流——排队等待（现有 20s 启动间隔语义保留）

### 9.4.3 账号删除/修改与状态机交互

- 删除账号：若其模拟器 BUSY → 事件 `account_removed` → 停止任务（杀 MAA 优雅）→ 模拟器 IDLE → 正常回收
- 修改 `emu_instance_index`：旧模拟器 release → 新模拟器 acquire（下次启动生效——运行中不改）
- 挂起（suspended）：BUSY 中挂起 → 当前轮跑完自然释放（不中途打断——除非用户要求立即停）

### 9.4.4 事件循环守护

- 事件循环线程心跳（每 10s 打点）+ 崩溃捕获（try/except 全包 + 日志 + **自动重启事件循环**——吸收"守护线程崩溃无声消失"教训）
