...
- UI 组件变量：简短命名（�?`sl` = status label�?

### 线程安全

- 所�?UI 操作必须在主线程执行
- 子线程通过 Qt Signal 向主线程发送数�?
- 避免在子线程中直接操�?`self.mw` �?UI 组件
...
- 为仓库条目添�?`guard_enabled`、`guard_max_restart`、`update_channel`、`launch_mode`、`account_ref`、`maa_type` �?
- 自动检�?MAA 类型（路径含 "MAA" �?`maa`�?
- 解析版本�?

### ADB 地址自动修复（v5 加载时）

`load_config()` 在加�?v5 配置时自动修复编码问题：

```python
"27.0.0.1" �?"127.0.0.1"  # 首字符丢失修�?
```
...
测试范围�?
- `make_id()` �?UUID 生成格式�? 位十六进制）
- `parse_maa_version()` �?从路径提取版本号
- `_version_tuple()` �?版本号比�?
- `get_platform_key()` �?平台标识
- `load_config()` �?`save_config()` �?重新加载一致�?
- v4 �?v5 迁移完整�?
- ADB 地址 sanitize 逻辑
...
通过 `app.setStyleSheet()` 全局应用。切换主题时调用 `_set_theme()`，重新设�?`QApplication` 的样式表�?`Palette`�?
